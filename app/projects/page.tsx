...
export default async function ProjectsPage() {
  ...
  const { data: projects, error } = await supabase
    .from("projects")
    .select("id, code, name, status, progress_pct, pm:profiles!pm_profile_id(display_name)")
    .order("created_at", { ascending: false });

  console.log("[ProjectsPage] projects count:", projects?.length, "error:", error);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Manage and track all your projects</p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-200 bg-slate-50">
            <tr>
              <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500">Code</th>
              <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500">Name</th>
              <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500">PM</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {(projects ?? []).map((project) => {
              const pm = project.pm as { display_name: string } | null;
              const progressPct = project.progress_pct ?? 0;
              return (
                <tr key={project.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-4">
                    <span className="font-mono text-sm font-semibold text-slate-900">{project.code}</span>
                  </td>
                  <td className="px-5 py-4">
                    <span className="font-medium text-slate-700">{project.name}</span>
                  </td>
                  <td className="px-5 py-4">
                    <span className="text-slate-600">{pm?.display_name ?? "—"}</span>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium capitalize ${getStatusBadge(project.status)}`}>
              ...
              <tr>
                <td className="px-5 py-12 text-center text-sm text-slate-500" colSpan={6}>
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <Link href="/projects/new" className="text-blue-600 hover:text-blue-700 font-medium">Create your first project →</Link>
                  </div>
                </td>
              </tr>
            )}
